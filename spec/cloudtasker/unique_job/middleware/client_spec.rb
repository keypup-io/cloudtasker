...
RSpec.describe Cloudtasker::UniqueJob::Middleware::Client do
  ...
  describe '#call' do
    ...
    let(:call_opts) { { some: 'opt' } }

    before { allow(Cloudtasker::UniqueJob::Job).to receive(:new).with(worker, call_opts).and_return(job) }
    ...
    it { expect { |b| middleware.call(worker, call_opts, &b) }.to yield_control }
  end
end
